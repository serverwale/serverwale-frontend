    ...
    <div className="p-4 md:p-8 bg-slate-50 min-h-screen">
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-lg overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-[#0F172A] to-[#1e293b] px-6 py-4">
          <h1 className="text-xl md:text-2xl font-bold text-white">Edit Blog</h1>
          <p className="text-blue-200 text-sm mt-1">Update your content with inline images</p>
        </div>
          ...
          <div className="border border-slate-200 rounded-xl overflow-hidden">
            <div className="bg-slate-50 px-4 py-3 border-b border-slate-200">
              <div className="flex flex-wrap items-center gap-1">
                ...
                <ToolbarButton onClick={() => insertTag("h2")} icon={Heading2} title="Heading 2" />
                <ToolbarButton onClick={() => insertTag("h3")} icon={Heading3} title="Heading 3" />
                <ToolbarButton onClick={() => insertTag("strong")} icon={Bold} title="Bold" />
                <ToolbarButton onClick={() => insertTag("em")} icon={Italic} title="Italic" />
                
                <div className="w-px h-6 bg-slate-300 mx-1" />
                
                <ToolbarButton onClick={() => insertTag("ul")} icon={List} title="Unordered List" />
                <ToolbarButton onClick={() => insertTag("ol")} icon={ListOrdered} title="Ordered List" />
                
                <div className="w-px h-6 bg-slate-300 mx-1" />
                
                <ToolbarButton onClick={() => insertTag("blockquote")} icon={Quote} title="Quote" />
                <ToolbarButton onClick={() => insertTag("code")} icon={Code} title="Code" />
                <ToolbarButton onClick={() => insertTag("a href='#'", "Link text")} icon={Link} title="Link" />
                ...
                <label
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-md cursor-pointer transition-all duration-200 ${
                    uploadingInlineImage
                      ...
                      : "bg-[#0055E5] text-white hover:bg-[#C2410C]"
                  }`}
                >
                  <Image size={16} />
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 px-4 py-2 bg-white border-2 border-dashed border-slate-300 rounded-lg cursor-pointer hover:border-[#0055E5] hover:bg-blue-50 transition-all">
                ...
                <span className="text-sm text-slate-600">{image ? image.name : "Change featured image"}</span>
                <input
                  type="file"
                  accept="image/*"
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-4 border-t border-slate-200">
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              ...
              <button
                ...
                disabled={saving}
                className="px-6 py-2.5 bg-[#0055E5] hover:bg-[#C2410C] text-white rounded-lg font-medium transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/30 disabled:opacity-50 disabled:cursor-not-allowed flex-1 sm:flex-none flex items-center justify-center gap-2"
              >
                {saving ? (
                  <>
